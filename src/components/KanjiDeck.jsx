  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 bg-slate-50 dark:bg-slate-900 min-h-screen">
      <header className="mb-10">
        <h1 className="font-display text-3xl md:text-4xl font-black text-slate-800 dark:text-slate-100 tracking-tight">{deckData.title}</h1>
          ...
          return (
            ...
            >
              ...
              <div className="p-6 grow space-y-6">
                <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b dark:border-slate-700 pb-2 mb-4">Vocabulary Context</h3>
                {item.usage_examples?.map((ex, exIdx) => (
                  <div key={exIdx} className={`relative pl-4 border-l-2 ${!isDue ? 'border-slate-200 dark:border-slate-600' : 'border-indigo-100 dark:border-indigo-700'}`}>
                    ...
                    <div className="flex justify-between items-end mb-2">
                      <span className="text-lg font-bold font-japanese dark:text-slate-200">
                        {ex.compound_word} <span className="text-xs font-normal text-slate-400 dark:text-slate-500 ml-1">({ex.reading_hiragana})</span>
                      </span>
                      <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">{ex.meaning_hinglish}</span>
                    </div>
                    {ex.example_sentence && (
                      <div
                        className={`p-3 rounded-xl border ${!isDue ? 'bg-slate-100 dark:bg-slate-700 border-slate-100 dark:border-slate-600' : 'bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-50/80 dark:border-indigo-800/40'}`}
                        onMouseUp={() => handleSentenceMouseUp(ex.example_sentence, item.id)}
                      >
                        <p className="text-sm font-japanese text-slate-800 dark:text-slate-200 mb-1 leading-relaxed cursor-text select-text">
                          {parseFuriganaString(ex.example_sentence)}
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 italic">{ex.sentence_meaning}</p>
                      </div>
                    )}
                  </div>
                ))}
